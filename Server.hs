      ...
      TextData str -> -- TODO, don't send messages to users being ignored
        ...
        case channel of
          ...
          Nothing -> do
            return st
      Login inputName ->
        let channel = M.lookup "General" (channelToUser st) in
        case channel of
          Just c -> do
            return (st { userToChannel = M.insert inputName "General" (userToChannel st),
                         channelToUser = M.insert "General" (inputName : c) (channelToUser st),
                         userToSocket = M.insert inputName sckt (userToSocket st),
                         socketToUser = (sckt, inputName) : socketToUser st })
          Nothing -> do
            return (st { userToChannel = M.insert inputName "General" (userToChannel st),
                         channelToUser = M.insert "General" [inputName] (channelToUser st),
                         userToSocket = M.insert inputName sckt (userToSocket st),
                         socketToUser = (sckt, inputName) : socketToUser st })
      Logout ->
        let channel = M.lookup uname (userToChannel st) in
